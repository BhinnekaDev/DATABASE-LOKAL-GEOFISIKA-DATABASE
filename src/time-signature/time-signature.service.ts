...
export class TimeSignatureService {
  ...
  constructor(
    ...
    if (!supabaseUrl || !supabaseKey) {
      throw new Error('Supabase URL atau Role Key tidak ditemukan.');
    }
  ...
  async saveTimeSignature(
    ...
    const { data: insertedTandaWaktu, error: insertError } = await this.supabase
      .from('time_signature')
      .insert({
        name,
        date,
  ...
  async deleteTimeSignature(
    ...
    // 4. Hapus data dari tabel time_signature
