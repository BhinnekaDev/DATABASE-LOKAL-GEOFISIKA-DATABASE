...
// Controller untuk fetch admin
const fetchAdmin = async (req, res) => {
    try {
        const hasil = await adminService.fetchAdmin();
        res.status(200).json(hasil);
    } catch (error) {
        // Kalau gagal, kirim pesan error
        res.status(400).json({ message: error.message });
...
// Controller untuk featch admin berdasarkan ID
const fetchAdminById = async (req, res) => {
    ...
    try {
        const hasil = await adminService.fetchAdminById(user_id);
        res.status(200).json(hasil);
    } catch (error) {
        // Kalau gagal, kirim pesan error
        res.status(400).json({ message: error.message });
